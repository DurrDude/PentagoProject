...
class Player:
                                    ...
                                    if board.board[y + yChange][x + xChange] == foundToken:
                                        ...
                                        if winPossible:
                                            ...
                                            for chainMultiple in chainMultipleList:
                                                if board.board[y + (chainMultiple * yChange)][x + (chainMultiple * xChange)] == foundToken:
    ...
    def miniMax(self, board, min, depth, maxDepth):
        ...
        # ---------------------------------------------------------------------------
        #print('running miniquax', depth,board)
        global nodesVisited
        if depth == 0:
            allMoves = []
        maxHeuristic = -(self.INFINITY + 1)
        oppToken = 'w' if self.token == 'b' else 'b'
        depthPlaceholder = depth
        for playerMove in board.getMoves():
            nodesVisited += 1
            stateOpponentBoard = board.applyMove(playerMove, self.token)
            if self.win(stateOpponentBoard):
                print('Win found. Move: {move}'.format(move=playerMove))
                return (playerMove, self.INFINITY-depth)
            minHeuristic = (self.INFINITY + 1)
            if depthPlaceholder + 1 < maxDepth:
                for oppMove in stateOpponentBoard.getMoves():
                    nodesVisited += 1
                    statePlayerBoard = stateOpponentBoard.applyMove(oppMove, oppToken)
                    #print('double rainbow',stateOpponentBoard,statePlayerBoard)
                    if self.opponent.win(statePlayerBoard):
                        #print('Opponent win found. Opponent move in response to {playerMove}= {move}'.format(playerMove=playerMove,move=oppMove))
                        minMove = oppMove
                        minHeuristic = -(self.INFINITY-depth)
                        break
                    if depthPlaceholder+2 < maxDepth:
                        throwawayVariable, oppMoveHeuristic = self.miniMax(statePlayerBoard, -10000,depth+2, maxDepth)
                        #print('Player\'s best response to opponent move {oppMove} is {responseMove} which leads to player heuristic of {playerHeuristic}. Current lowest heuristic is {min}'
                        #      .format(oppMove=oppMove,responseMove=throwawayVariable,playerHeuristic=oppMoveHeuristic, min=minHeuristic))
                    else:
                        oppMoveHeuristic = self.rd652_h(statePlayerBoard)
                    if oppMoveHeuristic < minHeuristic:
                        #print('Found lower heuristic for opponent at depth={depth}. Min heuristic for opponent changed to'.format(depth=depth+2), oppMoveHeuristic)
                        minHeuristic = oppMoveHeuristic
                        minMove = oppMove
                        ### alphaBeta
                        global nodesPruned
                        if oppMoveHeuristic <= maxHeuristic:
                            #print('{playerMove} node pruned. {move} move for opponent has beta < alpha. Alpha = {alpha}, Beta = {beta}'.format(playerMove=playerMove,move=oppMove,alpha=maxHeuristic,beta=oppMoveHeuristic))
                            nodesPruned += 1
                            break

                        ####
                '''
                print('Opponent\'s best move for player move {playerMove} is {oppBestMove}. Min heuristic = {oppHeuristic}. Max = {max}'.format(
                        playerMove=playerMove, oppBestMove=minMove, oppHeuristic=minHeuristic, max=maxHeuristic)
                )
                '''
            else:
                minHeuristic = self.rd652_h(stateOpponentBoard)
            #print('Resulting board:\n',stateOpponentBoard)
            if depth == 0:
                allMoves.append([playerMove,minHeuristic])
            if minHeuristic > maxHeuristic:
                #print('Found higher heuristic for player at depth={depth}. Max heuristic for player changed to'.format(depth=depth+1), minHeuristic)
                maxHeuristic = minHeuristic
                move = playerMove
        value = maxHeuristic

        if depth == 0:
            bestFiveMoves = sorted(allMoves,key=lambda x:x[1],reverse=True)[0:5]
            print('Best five moves and backed up values:',bestFiveMoves)
            bestMove,bestMoveHeuristic,record = self.monteCarloSearch(board, bestFiveMoves)
            print('Best move found from Monte Carlo and record: {move}, {record}'.format(move=bestMove,record=record))
            #print('All moves:', sorted(allMoves,key=lambda x:x[1],reverse=True))
            move = bestMove
            value = bestMoveHeuristic

        return move, value  # return move and backed-up value

    def monteCarloSearch(self,board,bestFiveMoves):
        monteMoveList = []
        print('Best five moves found. Running Monte Carlo Search.')
        for monteMove in bestFiveMoves:
            monteMovePlayoutPoints,record = self.playout(board.applyMove(monteMove[0], self.token))
            monteMoveList.append([monteMove[0], monteMove[1], monteMovePlayoutPoints,record])
        print('monte move list:',sorted(monteMoveList,key=lambda x:x[2], reverse=True))
        bestMove = sorted(monteMoveList, key=lambda x: x[2], reverse=True)[0]
        return bestMove[0],bestMove[1],bestMove[3]

    def playout(self, board):
        print(board,end='')
        #assumes you just made a move so plays with opponent first
        oppToken = 'b' if self.token == 'w' else 'w'
        points = 0
        numWins = 0
        numDraws = 0
        numLosses = 0
        resultsList = []
        for gameNum in range(0,150):
            playerPlayout = Player('playerPlayout','c',self.token)
            opponentPlayout = Player('opponentPlayout','c',oppToken)
            playoutPlayers = [playerPlayout, opponentPlayout]
            currentPlayer = 1
            pb = board
            ...
            ###
            while (not gameOver):
                #move = playoutPlayers[currentPlayer].playerMove(pb)
                moveList = pb.getMoves()
                #print(len(moveList))
                if len(moveList) != 0:
                    move = moveList[random.randint(0,len(moveList)-1)]
                    #print(playoutPlayers[currentPlayer].name + "'s move: " + move)
                    newBoard = copy.deepcopy(pb)
                    newBoard = newBoard.applyMove(move, player[currentPlayer].token)
                    #print(newBoard)
                numEmpty = numEmpty - 1
                win0 = playoutPlayers[0].win(newBoard)
                win1 = playoutPlayers[1].win(newBoard)
            ...
            if (win0 and win1):
                ...
                resultsList.append('D')
                numDraws += 1
        points = (numWins*2) + (numDraws*1)
        #print(resultsList)
        print(numWins, '-', numLosses,'-', numDraws,'\n\n\n\n\n')
        return points,'{numWins}-{numLosses}-{numDraws}'.format(numWins=numWins,numLosses=numLosses,numDraws=numDraws)
        ...
        while (not ValidMove):
            ...
            for move in moveList:
                if move == hMove:
                    ...
                    break
            if hMove == 'minimax':
                hMove, value = player[currentPlayer].miniMax(pb, -10000, 0, 2)
                ValidMove = True
            if (not ValidMove):
                print("Invalid move.  ")
        return hMove
    ...
    def getComputerMove(self, board):
        ...
        maxDepth = 2
        move, value = self.miniMax(board, -10000, 0, maxDepth)
        end_time = time.time()
        print('Time elapsed: {time} seconds'.format(time=round(end_time-start_time,5)))
        print('Lookahead depth: {maxDepth} moves'.format(maxDepth=maxDepth))
        return move
    ...
    def playerMove(self, board):
        ...
        if self.playerType == "human":
            ...
            print('Computer thinking of best move...')
...
if __name__ == "__main__":
    ...
    gameOver = False
    currentPlayer = 0
    player[currentPlayer].opponent = player[1-currentPlayer]
    player[1-currentPlayer].opponent = player[currentPlayer]
    print(pb)
    numEmpty = pb.emptyCells
    while (not gameOver):
        #####
        '''
        print('minimax')
        move, value = player[currentPlayer].miniMax(pb, -10000, 0, 3)
        print('miniimaxdone')
        '''
        #####
        global nodesVisited, nodesPruned
        nodesVisited,nodesPruned = 0, 0
        move = player[currentPlayer].playerMove(pb)
        if move == "exit":
            break
        print('Nodes Visited: {nodesVisited}\nNodes Pruned: {nodesPruned}'.format(nodesVisited=nodesVisited,nodesPruned=nodesPruned))
        ...
        for playerID in playerList:
            grabPlayer = player[playerID]
            print('{name} ({token})\'s heuristic: {heuristic}'.format(name=grabPlayer.name, token=grabPlayer.token,
                                                                      heuristic=grabPlayer.rd652_h(newBoard)))
        #####
